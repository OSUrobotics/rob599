...
public:
	...
	BasicService() :Node("service") {
		...
		// two parameters.
		service_ = this->create_service<Doubler>("doubler", std::bind(&BasicService::service_callback, this, _1, _2));
	}

	// A callback to handle the service request.  The has parameters for the request and
	// the response.
	void service_callback(
		const std::shared_ptr<Doubler::Request> request,
		const std::shared_ptr<Doubler::Response> response) {
...
private:
	...
	// message type.
	rclcpp::Service<Doubler>::SharedPtr service_;
};
